...
Tur is a lightweight, deterministic CLI that manages the lifecycle of your AI personas. It treats an AI identity as a structured, immutable software object.
...
2. If no default is set, it will launch an interactive Textual TUI asking you to select one from your available personas.
...
Bootstrap a new persona interactively. This creates the necessary `.tur/personas/` directory structure, generates a unique UUID, and creates your first `persona.yaml` DNA file.
...
**The Awakening:** Compiles the static DNA (`persona.yaml`), the User Context (`user.yaml`), and the Memory Bank into a complete "System Prompt" ready to be fed to an LLM.
...
**Dehydration:** Extracts insights, facts, and axioms from a raw chat log to update the long-term Memory Bank. It uses an LLM (acting as the Subconscious) to parse the log and structure the data.
...
Inspect the contents of the Memory Bank. This lists all active memories that will be included during the next `wake` cycle.
...
Calculates the "Constraint Dimensionality" ($C_p$) of a persona based on its principles and weights. This helps you understand if your persona's ruleset is becoming too complex for an LLM to handle reliably.
...
Archives a specific memory by its UUID, removing it from the active context window. The memory is moved to an `archive/` folder and is no longer loaded during `wake`.
...
Duplicates an existing persona into a new identity with a new UUID. This is useful for creating specialized branches of a persona without polluting the original's memory bank.
